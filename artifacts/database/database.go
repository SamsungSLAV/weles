...
func (aDB *ArtifactDB) Open(dbPath string) error {
	...
	if err != nil {
		logger.WithProperty("path", dbPath).WithError(err).Error("Failed to open db:", err.Error())
...
func (aDB *ArtifactDB) InsertArtifactInfo(ai *weles.ArtifactInfo) (err error) {
	...
	if err != nil {
		logger.WithError(err).WithProperty("ArtifactPath", string(ai.Path)).
...
func (aDB *ArtifactDB) SelectPath(path weles.ArtifactPath) (weles.ArtifactInfo, error) {
	...
	if err != nil {
		logger.WithError(err).WithProperty("ArtifactPath", string(path)).
			Error("Failed to slect artifact.")
...
func (aDB *ArtifactDB) Filter(filter weles.ArtifactFilter, sorter weles.ArtifactSorter,
	...
	defer func() {
		if err != nil {
			logger.WithError(err).Error("Failed to filter records.")
			// err should be logged when it occurs.
			if err2 := trans.Rollback(); err2 != nil {
				logger.WithError(err2).Alert("Transaction rollback failed. Database is unusable.")
			}
		}
	}()
	...
	if err != nil {
		logger.WithError(err).Error("Failed to get remaining records count.")
	...
	if err != nil {
		logger.WithError(err).Error("Failed to get total record count.")
		return nil, weles.ListInfo{}, errors.New(whileFilter + dbTotalFail + err.Error())
	}

	if tr == 0 {
		// err needs to be updated for deferred 'if err!=nil' to catch it and roll back
	...
	if err != nil {
		logger.WithError(err).Error("Failed to get artifacts.")
		return nil, weles.ListInfo{}, errors.New(whileFilter + dbArtifactInfoFail + err.Error())
	}
	if err := trans.Commit(); err != nil {
		logger.WithError(err).Error("Failed to commit transaction to db.")
...
func (aDB *ArtifactDB) SetStatus(change weles.ArtifactStatusChange) error {
	...
	if err != nil {
		// err logged in SelectPath
		return err
	}

	ai.Status = change.NewStatus
	if _, err = aDB.dbmap.Update(&ai); err != nil {
		logger.WithError(err).Alert("Failed to update database.")
