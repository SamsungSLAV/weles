...
func (h *ParserImpl) Parse(j weles.JobID) {
	...
	if err != nil {
		logger.WithError(err).WithProperty("JobID", j).
			Error("Failed to set job's status to PARSING.")
	...
	if err != nil {
		logger.WithError(err).WithProperty("JobID", j).Error("Failed to get job's yaml file")
	...
	if err != nil {
		logger.WithError(err).WithProperty("JobID", j).Error("Failed to create YAML file.")
	...
	if err != nil {
		logger.WithError(err).WithProperty("JobID", j).Error("Failed to write job's yaml to file.")
		h.SendFail(
	...
	if err != nil {
		logger.WithError(err).WithProperty("JobID", j).Error("Failed to parse yaml for job.")
