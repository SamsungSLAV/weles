...
func (h *BoruterImpl) pop(j weles.JobID) (r boruta.ReqID, err error) {
	...
	rinfo, ok := h.info[j]
	if !ok {
		logger.Errorf("tried to remove %d, but it wasnt found in BoruterImpl.info map:  ", j)
...
func (h *BoruterImpl) acquire(j weles.JobID, rinfo boruta.ReqInfo) {
	...
	if err != nil {
		h.remove(j, rinfo.ID)
		msg := fmt.Sprintf("Cannot acquire worker from Boruta : %s", err.Error())
		logger.Error(msg)
		h.SendFail(j, msg)
		return
	}
	...
	if err != nil {
		h.remove(j, rinfo.ID)
		msg := fmt.Sprintf("Internal Weles error while setting Dryad : %s", err.Error())
		logger.Error(msg)
		h.SendFail(j, msg)
		return
	}
...
func (h *BoruterImpl) loop() {
	...
	for {
		...
		if err != nil {
			logger.Error("Failed to list Boruta requests:", err.Error())
			continue
		}
...
func (h *BoruterImpl) Request(j weles.JobID) {
	...
	if err != nil {
		msg := fmt.Sprintf("Internal Weles error while changing Job status : %s", err.Error())
		logger.Error(msg)
		h.SendFail(j, msg)
		return
	}
	...
	if err != nil {
		msg := fmt.Sprintf("Internal Weles error while getting Job config : %s", err.Error())
		logger.Error(msg)
		h.SendFail(j, msg)
		return
	}
	...
	if err != nil {
		msg := fmt.Sprintf("Failed to create request in Boruta : %s", err.Error())
		logger.Error(msg)
		h.SendFail(j, msg)
		return
	}
...
func (h *BoruterImpl) Release(j weles.JobID) {
	r, err := h.pop(j)
	if err != nil {
		logger.Error("Failed to return Dryad to Boruta's pool:", err.Error())
		return
	}
